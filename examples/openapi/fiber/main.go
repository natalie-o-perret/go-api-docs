// Example: openapi/fiber - openapi.Router wrapped inside a Fiber app.
//
// gofiber/adaptor converts the openapi.Router (http.Handler) into a fiber
// handler.  A wildcard route forwards every request to the openapi.Router.
//
...
func main() {
	api := openapi.New(
		openapi.Info{Title: "Tasks API (fiber)", Version: "1.0.0"},
		openapi.WithServer("http://localhost:9095", "Local"),
	...
	// Fiber app with standard middleware.
	...
	app.Get("/docs", adaptor.HTTPHandler(ui))
	app.Get("/scalar.js", adaptor.HTTPHandler(ui))
	app.All("/*", adaptor.HTTPHandler(api))

	log.Println("listening on http://localhost:9095  (docs:/docs  spec:/openapi.json)")
