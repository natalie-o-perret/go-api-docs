// Example: openapi/gin - openapi.Router wrapped inside a gin engine.
//
// gin.WrapH converts the openapi.Router (http.Handler) into a gin HandlerFunc.
// A catch-all route forwards every request to the openapi.Router, which
// internally dispatches via its own stdlib mux and sets path values itself.
//
...
func main() {
	api := openapi.New(
		openapi.Info{Title: "Tasks API (gin)", Version: "1.0.0"},
		openapi.WithServer("http://localhost:9093", "Local"),
	...
	// gin engine with standard middleware.
	// gin.WrapH converts http.Handler to gin.HandlerFunc.
	// The openapi.Router internal stdlib mux handles dispatch and path params.
	g := gin.Default()

	g.GET("/docs", gin.WrapH(ui))
	g.GET("/scalar.js", gin.WrapH(ui))
	g.Any("/*path", gin.WrapH(api))

	log.Println("listening on http://localhost:9093  (docs:/docs  spec:/openapi.json)")
	log.Fatal(g.Run(":9093"))
}
