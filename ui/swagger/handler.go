...
// The JS bundle and CSS are vendored via go:embed but you can swap them by
// pointing WithJSPath / WithPresetPath / WithCSSPath at a CDN.
//
// Quick start:
//
...
// It serves the embedded JS bundle, standalone preset, and CSS at their
// configured paths; every other path gets the HTML page.
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	switch r.URL.Path {
	case h.cfg.jsPath:
		w.Header().Set("Content-Type", "application/javascript")
		w.Header().Set("Cache-Control", "public, max-age=31536000, immutable")
		_, _ = w.Write(assets.BundleJS)
	case h.cfg.presetPath:
		w.Header().Set("Content-Type", "application/javascript")
		w.Header().Set("Cache-Control", "public, max-age=31536000, immutable")
		_, _ = w.Write(assets.StandalonePresetJS)
	case h.cfg.cssPath:
		w.Header().Set("Content-Type", "text/css; charset=utf-8")
		w.Header().Set("Cache-Control", "public, max-age=31536000, immutable")
		_, _ = w.Write(assets.CSS)
	default:
