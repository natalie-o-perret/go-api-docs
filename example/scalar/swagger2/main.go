...
// specJSON is rawSpec with host/schemes rewritten so Scalar's "Try it"
// panel sends requests to this server instead of api.acme.example.com.
var specJSON = func() []byte {
	var doc map[string]any
	if err := json.Unmarshal(rawSpec, &doc); err != nil {
		panic("parse swagger.json: " + err.Error())
	}
	doc["host"] = "localhost:8081"
	doc["schemes"] = []string{"http"}
	b, err := json.MarshalIndent(doc, "", "  ")
	if err != nil {
		panic("re-marshal swagger.json: " + err.Error())
	}
	return b
}()
