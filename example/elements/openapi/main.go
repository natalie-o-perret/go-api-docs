// Example: elements/openapi — spec generated automatically from Go types and
// handler signatures, zero manual JSON required.
//
// Uses github.com/nopereta/go-api-docs/openapi for typed route registration
// and spec generation, then serves the result via the elements package.
//
// Run from this directory:
//
//	go run .
//	open http://localhost:9085
package main
...
func main() {
	r := openapi.New(
		...
		openapi.WithServer("http://localhost:9085", "Local dev"),
	...
	ui, err := elements.New(
		...
		elements.WithPageTitle("Tasks API - Stoplight Elements"),
	...
	mux.Handle("/elements.js", ui)
	mux.Handle("/elements.css", ui)
	mux.Handle("/docs", ui)
	mux.Handle("/", r)

	log.Println("listening on http://localhost:9085")
	log.Println("  docs:  http://localhost:9085/docs")
	log.Println("  spec:  http://localhost:9085/openapi.json")
	log.Fatal(http.ListenAndServe(":9085", mux))
}
